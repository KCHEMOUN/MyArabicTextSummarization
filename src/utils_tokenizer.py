...
def apply_tokenizer(tokenizer, text, sum, max_len):
	...
	##ANOTHE POSSIBLITY for the code
	input_text = f"<BOS> {text} {sum} <EOS>"
	...
	return tokenizer(input_text, truncation=True, max_length=max_len, padding="max_length")


# return tokenizer('<BOS>' {text} {sum} '<EOS>', truncation=True, max_length=max_len, padding="max_length")
